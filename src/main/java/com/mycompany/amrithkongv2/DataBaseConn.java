...
public final class DataBaseConn {
    ...
    private static final String URL = "jdbc:derby:RussianRDB;create=true";  //embedded DB
    ...
    public DataBaseConn() 
    {
        //doing a singleton DB since this is the pattern for DB
        try 
        {
    ...
    public void createMemorialTable() // Creates a table that logs new players, does not update existing player or will add old players
    {
        ...
        catch (SQLException e) // you may see XOY32 similar in other methods, this handles duplicate table. Had ot
        {
            if (!"X0Y32".equals(e.getSQLState())) 
            {
                System.out.println("Error creating MemorialTable" );
            }
        }
    }
    ...
    {
        //Create the table that holds all the records from the first game session
        try (Statement smt = conn.createStatement()) 
        {
            smt.executeUpdate("""
            ...
            if (!"X0Y32".equals(e.getSQLState())) {
                System.out.println("Error creating HistoricalTable");
            }
        }
    ...
    {
        //Only holds information from the previous game played
        try (Statement smt = conn.createStatement()) 
        {
            smt.executeUpdate("""
        ...
        catch (SQLException e) 
        {
            if (!"X0Y32".equals(e.getSQLState())) 
            {
                System.out.println("Error creating RecentTable" );
            }
        }
    }
